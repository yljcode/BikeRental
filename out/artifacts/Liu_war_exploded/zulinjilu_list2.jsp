...
//txixixngdy
    ...
    String url = "zulinjilu_list2.jsp?1=1"; 
    String sql =  "select * from zulinjilu where zulinren='"+request.getSession().getAttribute("username")+"' ";
...
//youzuiping2
 ...
 <td width='80' align='center'><%=map.get("issh")%></td>
    ...
    <td width="60" align="center"><a href="zulinjilu_updt.jsp?id=<%=map.get("id")%>">修改</a>  <a href="zulinjilu_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a href="zulinjiludetail.jsp?id=<%=map.get("id")%>" target="_blank">详细</a>  <a href="guihuanjilu_add.jsp?id=<%=map.get("id")%>" >归还</a></td>
  </tr>
  	<%
  }
   %>
